...
- 四条学习路径：零基础入门、编程、Agent / RAG、AI 部署与交付
- 国内外模型对比：按应用场景理解模型差异
- 作品样例库：展示学习成果应该长什么样
- GitHub AI 学习资源雷达：标注值得借鉴、可按许可证复用、需谨慎改造的开源项目，支持搜索、处理建议筛选、许可证筛选、复制资源卡和复制贡献模板
- AI 项目挑战库：12 个带难度、时间、工具、交付物和验收标准的项目挑战，支持搜索、难度筛选和复制挑战模板
...
- 资源雷达区会标注 SPDX 许可证、复核日期与处理策略；支持复制资源卡和贡献模板，许可证不清晰或限制较强的项目只借鉴机制，不搬运原文。
